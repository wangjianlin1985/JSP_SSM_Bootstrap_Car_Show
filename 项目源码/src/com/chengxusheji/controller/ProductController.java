...
public class ProductController extends BaseController {
	...
	public void list(String addTime,@ModelAttribute("productClassObj") ProductClass productClassObj,String productName,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (addTime == null) addTime = "";
		if (productName == null) productName = "";
		if(rows != 0)productService.setRows(rows);
		List<Product> productList = productService.queryProduct(addTime, productClassObj, productName, page);
	    /*计算总的页数和总的记录数*/
	    productService.queryTotalPageAndRecordNumber(addTime, productClassObj, productName);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String addTime,@ModelAttribute("productClassObj") ProductClass productClassObj,String productName,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (addTime == null) addTime = "";
		if (productName == null) productName = "";
		List<Product> productList = productService.queryProduct(addTime, productClassObj, productName, currentPage);
	    /*计算总的页数和总的记录数*/
	    productService.queryTotalPageAndRecordNumber(addTime, productClassObj, productName);
	    /*获取到总的页码数目*/
	    int totalPage = productService.getTotalPage();
	...
	public void OutToExcel(String addTime,@ModelAttribute("productClassObj") ProductClass productClassObj,String productName, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(addTime == null) addTime = "";
        if(productName == null) productName = "";
        List<Product> productList = productService.queryProduct(addTime,productClassObj,productName);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Product信息记录"; 
        String[] headers = { "商品id","商品类别","商品名称","商品主图","商品价格","商品库存","发布时间"};
